    ...
    public class Photo : IEnumerable
    {
        ...
        public void Render(SpriteBatch batch, bool isFiltering, bool isShadowing, Texture2D textureShadow,bool isWhiteframe, bool isDragged, Texture2D textureFrame, bool isFavorite)
        {
            if (isFiltering)
            {
                ...
                //Console.WriteLine(scaleTarget_);
            }
            else
            {
            ...
            if (scaleDisplay_ > 0)
            {
                if (texture_ != null)
                {
                    ...
                    if (isShadowing)
                    {
                        ...
                        {
                            float scalex = (Width * scaleDisplay_ + (float)((1d - Math.Exp(-(double)scaleDisplay_)) * (double)ResourceManager.MAR * 2d + 2d)) * 1.5f / 100f;
                            float scaley = (Height * scaleDisplay_ + (float)((1d - Math.Exp(-(double)scaleDisplay_)) * (double)ResourceManager.MAR * 2d + 2d)) * 1.5f / 100f;
                    ...
                    if (isWhiteframe)
                    {
                        ...
                        {
                            float scalex = (Width * scaleDisplay_ + (float)((1d - Math.Exp(-(double)scaleDisplay_)) * (double)ResourceManager.MAR * 2d + 2d)) / 2f;
                            float scaley = (Height * scaleDisplay_ + (float)((1d - Math.Exp(-(double)scaleDisplay_)) * (double)ResourceManager.MAR * 2d + 2d)) / 2f;
