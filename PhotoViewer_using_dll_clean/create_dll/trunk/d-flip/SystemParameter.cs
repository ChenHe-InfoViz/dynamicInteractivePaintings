...
namespace dflip
{
    public class SystemParameter
    {
        ...
        public static GraphicsAdapter graphicsAdapter = null;
        ...
        public void update()
        {
            ...
            {
                inputController.resetDock();
                strokeGroup.resetStrokes();
            }
            ...
            //update the display of system state
            if (dock_.touchState == PieMenu.PieMode.DragPhoto)
                SystemParameter.title = "Drag Photo";
            if (dock_.touchState == PieMenu.PieMode.DrawLine)
                SystemParameter.title = "Draw Line";
            if (dock_.touchState == PieMenu.PieMode.MoveLine)
                SystemParameter.title = "Transform Line"; 
            if (dock_.touchState == PieMenu.PieMode.DeletePhoto)
                SystemParameter.title = "Delete Photo";
        ...
        public void draw()
        {
            ...
            //SystemParameter.graphicsDevice.SetRenderTarget(null);
            ...
            if (systemState.curState == SystemState.ATTRACTOR_GEOGRAPH)
            {
                ...
                {
                    Vector2 mapScale = new Vector2(width / (float)ResourceManager.mapTex_tohoku.Width, height / (float)ResourceManager.mapTex_tohoku.Height);
                    //batch_.Begin();
                    batch_.Draw(ResourceManager.mapTex_tohoku, Vector2.Zero, null, halfWhite, 0f, Vector2.Zero, mapScale, SpriteEffects.None, 1f);
                    //batch_.End();
                }
                else
                {
                    halfWhite = new Color(255, 255, 255, (byte)122);
            ...
            //batch_.End();
            //RenderTarget2D renderTarget = new RenderTarget2D(SystemParameter.graphicsDevice, graphicsDevice.PresentationParameters.BackBufferWidth, graphicsDevice.PresentationParameters.BackBufferHeight, mipMap: true, preferredFormat: SurfaceFormat.Color, preferredDepthFormat: DepthFormat.None);

            //BlendState blendState = BlendState.Opaque;

            //SystemParameter.graphicsDevice.SetRenderTarget(renderTarget);
            //render photos
            //batch_.Begin(SpriteSortMode.BackToFront, blendState);
