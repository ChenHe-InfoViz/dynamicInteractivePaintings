...
namespace dflip.Element
{
    public class Stroke
    {
        ...
        // stroke interval
        public Vector2 deleteIcon
        {
            get;
            set;
        }

        public List<Vector2> insideStrokes_ = new List<Vector2>();
        public List<Vector2> outsideStrokes_ = new List<Vector2>();
        ...
        public BoundingBox2D barBounding
        {
            get;
            set;
        }
        ...
        public void EnlargeStroke(int index, Vector2 v)
        {
            ...
            Vector2 baseInsideV = insideStrokes_[index] - center_;
            Vector2 baseOutsideV = outsideStrokes_[index] - center_;

            float baseLen = basev.Length();
            float baseInsideLen = baseInsideV.Length();
            float baseOutsideLen = baseOutsideV.Length();

            {
                basev.Normalize();
                baseInsideV.Normalize();
                baseOutsideV.Normalize();

                float length = basev.X * v.X + basev.Y * v.Y;
                float lengthInside = baseInsideV.X * v.X + baseInsideV.Y * v.Y;
                float lengthOutside = baseOutsideV.X * v.X + baseOutsideV.Y * v.Y;

                //
                float lengthh = basev.X * v.Y - basev.Y * v.X;
                float lengthhInside = baseInsideV.X * v.Y - baseInsideV.Y * v.X;
                float lengthhOutside = baseOutsideV.X * v.Y - baseOutsideV.Y * v.X;
                Vector2 basevv = new Vector2(-basev.Y, basev.X);
                Vector2 basevvInside = new Vector2(-baseInsideV.Y, baseInsideV.X);
                Vector2 basevvOutside = new Vector2(-baseOutsideV.Y, baseOutsideV.X);
                basevv *= lengthh;
                basevvInside *= lengthhInside;
                basevvOutside *= lengthhOutside;
                //
                for (int i = 0; i < sc; ++i)
                {
                    Vector2 dir = strokes_[i] - center_;
                    Vector2 dirInside = insideStrokes_[i] - center_;
                    Vector2 dirOutside = outsideStrokes_[i] - center_;

                    float dirLen = dir.Length();
                    float dirLenInside = dirInside.Length();
                    float dirLenOutside = dirOutside.Length();

                    dir.Normalize();
                    dirInside.Normalize();
                    dirOutside.Normalize();

                    strokes_[i] += dir * length * dirLen / baseLen;
                    insideStrokes_[i] += dirInside * lengthInside * dirLenInside / baseInsideLen;
                    outsideStrokes_[i] += dirOutside * lengthOutside * dirLenOutside / baseOutsideLen;

                    strokes_[i] += basevv;
                    insideStrokes_[i] += basevvInside;
                    outsideStrokes_[i] += basevvOutside;
                    
                }
                strokes_[sc] = strokes_[0];
                insideStrokes_[sc] = insideStrokes_[0];
                outsideStrokes_[sc] = outsideStrokes_[0];
                center_ += basevv;
            }
        }
        ...
        public void End()
        {
            ...
            //Console.WriteLine(center_);
            strokes_.Add(strokes_[0]);
            isClosed_ = true;

            //calculate stroke inside and out
            int gap = 100;
            foreach (Vector2 s in strokes_)
            {
                float x = s.X - center_.X;
                float y = s.Y - center_.Y;
                double para = Math.Sqrt(x * x + y * y);
                if (gap < para)
                {
                    para = gap / para;
                    insideStrokes_.Add(new Vector2((float)(center_.X + x * (1 - para)), (float)(center_.Y + y * (1 - para))));
                    outsideStrokes_.Add(new Vector2((float)(center_.X + x * (1 + para)), (float)(center_.Y + y * (1 + para))));
                }
                else
                {
                    insideStrokes_.Add(s);
                    outsideStrokes_.Add(s);
                }
            }
        }
        ...
        public bool IsInternal(Vector2 target, List<Vector2> strokes_)
        {
            int trueCount = 0;
        ...
        public Vector2 renderTag()
        {
            ...
            if (this.tags_.Count > 0)
            {
                ...
                {
                    int width = 5;
                    //ResourceManager.font_.Characters;
                    ...
                    SystemParameter.batch_.Draw(ResourceManager.frameSquare_, new Rectangle((int)this.Strokes[strokes_.Count - 2].X, (int)this.Strokes[strokes_.Count - 2].Y, width, height), Color.White);
                    width = 5;
                    foreach (var tag in tags_)
                    {
                        SystemParameter.batch_.DrawString(ResourceManager.font_, tag, new Vector2(this.Strokes[strokes_.Count - 2].X + width, strokes_[strokes_.Count - 2].Y), this.Color);
                        width += (int)ResourceManager.font_.MeasureString(tag).X + 5;
                    }
        ...
        public void Render()
        {
            if (strokes_.Count > 0)
            {
                ...
                for (int i = 0, len = strokes_.Count; i < len - 1; ++i)
                {
                    ...
                    {
                        SystemParameter.batch_.Draw(ResourceManager.stroke_, strokes_[i] - ((float)ResourceManager.stroke_.Width * 0.5f) * Vector2.One + v * (float)j, color_);
                    }
                }
                if (!isClosed_)
                {
                    ...
                    {
                        SystemParameter.batch_.Draw(ResourceManager.stroke_, strokes_[last] - ((float)ResourceManager.stroke_.Width * 0.5f) * Vector2.One + v * (float)j, color_);
                        SystemParameter.batch_.Draw(ResourceManager.stroke_, strokes_[last] - ((float)ResourceManager.stroke_.Width * 0.5f) * Vector2.One + v * (float)(j + 2), color_);
                        SystemParameter.batch_.Draw(ResourceManager.stroke_, strokes_[last] - ((float)ResourceManager.stroke_.Width * 0.5f) * Vector2.One + v * (float)(j + 4), color_);
                    }
                }
                //else
                //{
                //    SystemParameter.batch_.Draw(ResourceManager.batsuTex_, deleteIcon, Color.White);
                //}
                //b.End();
            }
        }
            ...
            if (strokes_.Count > 0)
            {
                ...
                for (int i = 0, len = strokes_.Count - 1; i < len; ++i)
                {
                    ...
                    for (int j = 0; j < jlen; ++j)
                    {
                        ...
                        SystemParameter.batch_.Draw(ResourceManager.stroke_, new Vector2((float)x, (float)y) - ((float)ResourceManager.stroke_.Width * 0.5f) * Vector2.One, color_);
                    }
                }
                //b.End();
            }
